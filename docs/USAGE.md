...
- Файлы: `report_HOSTNAME_YYYY-MM-DD_HH-MM.html`

### Сводный отчёт (index.html)

После генерации всех отчётов автоматически создаётся файл `index.html` со ссылками на все отчёты в каталоге.

**Пример сводного отчета:**

```
📊 Сводный отчёт по серверам
├── 📅 2024-01-15 (5 отчётов, 3 ошибки, 10 предупреждений)
│   ├── 🌐 prod.example.com
│   │   ├── server1.prod.example.com — ❌ 2 ошибки
│   │   └── server2.prod.example.com — ✅ OK
│   └── 🌐 staging.example.com
│       └── db1.staging.example.com — ⚠️ 5 предупреждений
└── 📅 2024-01-14 (3 отчёта, 1 ошибка, 2 предупреждения)
    └── ...
```
